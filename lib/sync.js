...
const { RETIRED_CORE_SKILL_FILES } = require('./skill');
...
function checkSkillsSync(target) {
  ...
  const retired = RETIRED_CORE_SKILL_FILES || [];
  for (const file of fs.readdirSync(srcDir)) {
    if (!file.endsWith('.md')) continue;
    if (retired.includes(file)) {
      outOfSync.push({
        path: path.join('.agent-room', 'skills', file).replace(/\\/g, '/'),
        reason: 'deprecated'
      });
      continue;
    }
    ...
    const relativeDest = path.relative(target, dest).replace(/\\/g, '/');
  ...
  if (fs.existsSync(claudeSkillsDir)) {
    for (const dirName of fs.readdirSync(claudeSkillsDir)) {
      const fullDir = path.join(claudeSkillsDir, dirName);
      if (fs.statSync(fullDir).isDirectory() && !activeSkills.has(dirName)) {
        outOfSync.push({ path: path.relative(target, fullDir).replace(/\\/g, '/'), reason: 'orphaned' });
      }
    }
  }
...
function purgeDeprecatedSkills(target) {
  const srcDir = path.join(target, '.agent-room', 'skills');
  const claudeSkillsDir = path.join(target, '.claude', 'skills');
  const purged = [];
  if (!fs.existsSync(srcDir)) return purged;
  ...
  for (const file of retired) {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        purged.push(path.join('.agent-room', 'skills', file).replace(/\\/g, '/'));
      } catch (err) {
        // ignore
      }
    }
    ...
    if (fs.existsSync(claudeSkillDir)) {
      try {
        ...
        purged.push(path.join('.claude', 'skills', skillName).replace(/\\/g, '/'));
      } catch (err) {
        // ignore
      }
    }
...
const CLAUDE_COMMANDS_TEMPLATE_DIR = path.join(ADAPTERS_TEMPLATE_DIR, 'claude-commands');

function checkClaudeCommandsSync(target) {
  const outOfSync = [];
  for (const cmdFile of CLAUDE_COMMAND_FILES) {
    const tmplPath = path.join(CLAUDE_COMMANDS_TEMPLATE_DIR, cmdFile);
    if (!fs.existsSync(tmplPath)) continue;
    const dest = path.join(target, '.claude', 'commands', cmdFile);
    const relativeDest = path.join('.claude', 'commands', cmdFile).replace(/\\/g, '/');
...
function syncSkillsToClaude(target) {
  ...
  if (fs.existsSync(srcDir)) {
    for (const file of fs.readdirSync(srcDir)) {
      ...
      const skillName = file.replace(/\.md$/, '');
      activeSkills.add(skillName);
      const dest = path.join(claudeSkillsDir, skillName, 'SKILL.md');
...
function syncClaudeCommands(target, force) {
  const results = [];
  for (const cmdFile of CLAUDE_COMMAND_FILES) {
    const tmplPath = path.join(CLAUDE_COMMANDS_TEMPLATE_DIR, cmdFile);
    if (!fs.existsSync(tmplPath)) continue;
    ...
    fs.mkdirSync(path.dirname(dest), { recursive: true });
    fs.writeFileSync(dest, tmplContent);
...
function runSync(target, args) {
  ...
  if (checkOnly) {
    ...
    const outOfSync = [];
    if (wantClaude) {
      outOfSync.push(...checkSkillsSync(target));
      outOfSync.push(...checkClaudeCommandsSync(target));
    }
  ...
  const purgedLegacy = purgeDeprecatedSkills(target);
  for (const item of purgedLegacy) {
    console.log(`  purged deprecated  ${item}`);
  }
  ...
  if (wantClaude) {
    ...
    if (fs.existsSync(claudeSkillsDir)) {
      for (const dirName of fs.readdirSync(claudeSkillsDir)) {
        ...
        if (fs.statSync(fullDir).isDirectory() && !activeSkills.has(dirName)) {
          fs.rmSync(fullDir, { recursive: true, force: true });
          console.log(`  removed orphaned  .claude/skills/${dirName}`);
        }
      }
    }

    results.push(...syncClaudeCommands(target, force));
  }
...
module.exports = {
  ...
  isGitDirty,
  checkClaudeCommandsSync,
  syncClaudeCommands,
  purgeDeprecatedSkills,
