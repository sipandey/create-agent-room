...
function getFindings(target) {
  ...
  return { notSetUp: false, critical, advisory, isMinimalProfile };
}

function runDoctor(target) {
  console.log(bold(`create-agent-room doctor: ${cyan(target)}\n`));

  const { notSetUp, critical, advisory, isMinimalProfile } = getFindings(target);
...
module.exports = { runDoctor, getFindings };
