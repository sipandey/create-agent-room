...
const { resolveHooksDir, installHooks } = require('./hook');
...
function checkHookDrift(target) {
  ...
  for (const hook of STATIC_HOOK_FILES) {
    ...
    if (hook.installed.includes('pre-commit')) {
      ...
      installedPath = path.join(hooksDir, 'pre-commit');
    } else {
      installedPath = path.join(target, ...hook.installed);
    }
    ...
    if (normalize(installedContent) !== normalize(templateContent)) {
      if (hook.installed.includes('pre-commit')) {
        const start = '# --- create-agent-room hook: pre-commit ---';
        const end = '# --- end create-agent-room hook: pre-commit ---';
        const startIdx = installedContent.indexOf(start);
        const endIdx = installedContent.indexOf(end);
        if (startIdx !== -1 && endIdx !== -1) {
          const body = installedContent.substring(startIdx + start.length, endIdx).trim();
          const expectedBody = templateContent.replace(/^#![^\n]*/, '').trim();
          if (normalize(body) === normalize(expectedBody)) {
            continue;
          }
        }
      }
...
function checkDeprecatedSkills(target) {
  const issues = [];
  ...
  for (const file of retired) {
    if (fs.existsSync(path.join(skillsDir, file))) {
      issues.push(
        `Deprecated legacy skill found: ${path.join('.agent-room', 'skills', file).replace(/\\/g, '/')} (superseded by RPI pipeline; run doctor --fix to remove)`
      );
    }
    const skillName = file.replace(/\.md$/, '');
    if (fs.existsSync(path.join(claudeSkillsDir, skillName))) {
      issues.push(
        `Deprecated mirrored skill found: ${path.join('.claude', 'skills', skillName).replace(/\\/g, '/')} (superseded by RPI pipeline; run doctor --fix to remove)`
      );
    }
  }
  return issues;
...
function getFindings(target) {
  ...
  const deprecatedSkillIssues = checkDeprecatedSkills(target);
...
function fixFindings(target) {
  ...
  for (const hook of STATIC_HOOK_FILES) {
    ...
    if (fs.existsSync(installedPath)) {
      ...
      if (normalize(installedContent) !== normalize(templateContent)) {
        if (hook.installed.includes('pre-commit')) {
          installHooks(target, { hooks: ['pre-commit'] });
        } else {
  ...
  // 4. Purge deprecated legacy skills from .agent-room/skills/ and .claude/skills/
  const skillsDir = path.join(target, '.agent-room', 'skills');
  const claudeSkillsDir = path.join(target, '.claude', 'skills');
  const retired = RETIRED_CORE_SKILL_FILES || [];
  ...
  for (const file of retired) {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        fixed.push(`Removed deprecated legacy skill: ${path.join('.agent-room', 'skills', file).replace(/\\/g, '/')}`);
      } catch (err) {
        // ignore
      }
    }
    ...
    if (fs.existsSync(claudeSkillDir)) {
      try {
        ...
        fixed.push(`Removed deprecated mirrored skill: ${path.join('.claude', 'skills', skillName).replace(/\\/g, '/')}`);
      } catch (err) {
        // ignore
      }
    }
