...
function resolveChecksToRun(options) {
  ...
  // Check if --skip was passed as a list or string
  const skipList = [];
  if (options.skip) {
    const list = Array.isArray(options.skip) ? options.skip : String(options.skip).split(',');
    for (const item of list) {
      const normalized = item.trim().toLowerCase();
      if (normalized === 'lint-sessions' || normalized === 'session' || normalized === 'sessions') {
        skipList.push('sessions');
      } else if (normalized === 'pr-audit' || normalized === 'pr') {
        skipList.push('pr');
      } else if (AVAILABLE_CHECKS.includes(normalized)) {
        skipList.push(normalized);
      }
    }
  }
...
async function runCiCli(target, args) {
  args = args || {};
  ...
  // Handle PR Sticky Comment if requested
  if (args.comment || args['pr-comment']) {
    try {
      const markdown = formatCiReport(report, 'markdown');
      const commentRes = await postPrComment(markdown, {
        target,
        token: args.githubToken || args['github-token'] || process.env.GITHUB_TOKEN || process.env.GH_TOKEN,
        prNumber: args.pr || args.prNumber || args['pr-number'],
      });
      if (commentRes.ok && !commentRes.skipped) {
        if (format === 'text') {
          console.log(`\n💬 PR compliance scorecard ${commentRes.action}: ${commentRes.url || '#' + commentRes.commentId}`);
        }
      } else if (commentRes.skipped) {
        if (format === 'text' && args.verbose) {
          console.log(`\n⚪ PR comment skipped: ${commentRes.message}`);
        }
      } else if (!commentRes.ok) {
        console.error(`\n⚠️ Failed to post PR comment: ${commentRes.error || commentRes.message}`);
      }
    } catch (err) {
      if (args.verbose) {
        console.error(`\n⚠️ PR comment error: ${err.message}`);
      }
    }
  }
