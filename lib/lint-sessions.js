...
const PLACEHOLDER_DECISIONS_RE =
  /^(none|n\/a|na|-|—|\.{3}|no decisions?|nothing|nope)\.?$/i;

function isPlaceholderDecisionsText(text) {
  const lines = text
    .trim()
    .split('\n')
    .map((line) => line.trim())
  ...
  return lines.every((line) => {
    ...
    return !content || PLACEHOLDER_DECISIONS_RE.test(content);
  });
}

function validateDecisionsMadeSection(fileName, content, errors) {
  const decisionsMatch = content.match(/##\s+Decisions made\r?\n+([\s\S]*?)(?=##|$)/);
  if (!decisionsMatch || !decisionsMatch[1].trim()) {
    errors.push(`${fileName}: Decisions made section is empty`);
    return;
  }

  const outcomeMatch = content.match(/##\s+Outcome[^#]*\*\*Status:\*\*\s*(.+?)(?:\n|\r)/);
  const outcome = outcomeMatch ? outcomeMatch[1].trim() : '';
  if (outcome === 'Completed' && isPlaceholderDecisionsText(decisionsMatch[1])) {
    errors.push(
      `${fileName}: Decisions made cannot be a placeholder when Outcome Status is Completed — record a real decision or note that decisions.md was updated`
    );
  }
}
...
function validateMarkdownSession(filePath, fileName) {
  ...
  try {
    ...
    if (!filesMatch || !filesMatch[1].trim()) {
      ...
      if (!filesText.match(/-\s*(Read|Created|Modified):/i)) {
    ...
    // Validate Decisions made (structure + substance when Completed)
    validateDecisionsMadeSection(fileName, content, errors);
...
function lintSessions(target) {
  ...
  if (!fs.existsSync(sessionsDir)) {
    return {
      ...
      totalWarnings: 0,
      errors: [],
      warnings: [`No sessions directory found at ${sessionsDir}`],
      results: []
    };
  }
  ...
  if (files.length === 0) {
    return {
      ...
      totalWarnings: 0,
      errors: [],
      warnings: [`No session logs found in ${sessionsDir}`],
      results: []
    };
  }
  ...
  for (const file of files) {
    ...
    for (const e of result.errors) allErrors.push(`${file}: ${e}`);
