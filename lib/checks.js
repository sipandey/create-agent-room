...
function collectFindings(target) {
  ...
  if (fs.existsSync(path.join(target, '.agent-room'))) {
    ...
    // 4. Lint RPI Artifacts (docs/research and docs/plans)
    const checkArtifactFrontmatter = (dirRelPath, file, artifactType, requiredKeys, numericKeys = []) => {
      const nameRegex = /^\d{4}-\d{2}-\d{2}-.+\.md$/;
      if (!nameRegex.test(file)) {
        errors.push(`${artifactType} artifact "${path.join(dirRelPath, file)}" must follow YYYY-MM-DD-<topic>.md naming convention`);
        return;
      }

      const filePath = path.join(target, dirRelPath, file);
      try {
        ...
        if (!match) {
          errors.push(`${artifactType} artifact "${path.join(dirRelPath, file)}" is missing valid YAML frontmatter delimiters (---)`);
          return;
        }
        ...
        for (const line of lines) {
          ...
          const separatorIndex = line.indexOf(':');
          if (separatorIndex !== -1 && !line.startsWith(' ') && !line.startsWith('\t')) {
            const key = line.slice(0, separatorIndex).trim();
            const val = line.slice(separatorIndex + 1).trim();
            metadata[key] = val;
            currentKey = key;
        ...
        for (const key of requiredKeys) {
          if (metadata[key] === undefined) {
            errors.push(`${artifactType} artifact "${path.join(dirRelPath, file)}" is missing the "${key}" metadata attribute in its frontmatter`);
          } else {
            const cleanVal = metadata[key].replace(/['"[\]-]/g, '').trim();
            if (!cleanVal) {
              errors.push(`${artifactType} artifact "${path.join(dirRelPath, file)}" has an empty "${key}" attribute`);
            }
          }
        }

        for (const numKey of numericKeys) {
          if (metadata[numKey] !== undefined) {
            const raw = metadata[numKey].replace(/['"]/g, '').trim();
            if (raw !== '' && isNaN(Number(raw))) {
              errors.push(`${artifactType} artifact "${path.join(dirRelPath, file)}" frontmatter "${numKey}" must be a number`);
            }
          }
        }
      } catch (err) {
        errors.push(`Failed to read or parse ${artifactType.toLowerCase()} artifact ${file}: ${err.message}`);
      }
    };

    const researchDir = path.join(target, 'docs/research');
    if (fs.existsSync(researchDir) && fs.statSync(researchDir).isDirectory()) {
      const files = fs.readdirSync(researchDir).filter((f) => f.endsWith('.md') && f !== 'README.md');
      for (const file of files) {
        checkArtifactFrontmatter(
          'docs/research',
          file,
          'Research',
    ...
    if (fs.existsSync(plansDir) && fs.statSync(plansDir).isDirectory()) {
      const files = fs.readdirSync(plansDir).filter((f) => f.endsWith('.md') && f !== 'README.md');
      for (const file of files) {
        checkArtifactFrontmatter(
          'docs/plans',
          file,
          'Plan',
          ['date', 'research_doc', 'branch', 'status', 'phases_total', 'phases_completed'],
