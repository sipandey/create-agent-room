...
function getSessionDir() {
  return path.join(process.cwd(), '.agent-room', 'sessions');
}
...
class SessionLog {
  ...
  toMarkdown() {
    ...
    if (files.length === 0) {
      files.push('- Modified: none');
    }

    const actions = this.actions.length > 0
      ? this.actions.map((a, i) => `${i + 1}. ${a}`).join('\n')
      : '1. Initialized session and reviewed workspace.';

    let decisions = 'None documented.';
    if (this.decisions.length > 0) {
      decisions = this.decisions.map((d) => `- ${d}`).join('\n');
    } else if (this.outcome === 'Completed') {
      decisions = '- Documented architectural decisions in .agent-room/decisions.md';
    }

    const handoff = this.handoffNote ? `\n\n**Handoff note (if applicable):**\n${this.handoffNote}` : '';
    const goalText = (this.goal || 'Complete session task and maintain workspace invariants.').trim().split('\n')[0].trim();
    const testCmd = this.tests && this.tests.command ? this.tests.command : 'None';
    const testRes = this.tests && this.tests.result ? this.tests.result : 'Not executed';
...
`;
  ...
  save(format = 'markdown') {
    ...
    const filePath = path.join(sessionDir, fileName + ext);
