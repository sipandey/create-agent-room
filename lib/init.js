...
const VALID_TOOLS = ['claude', 'cursor', 'windsurf', 'cline', 'codex', 'copilot', 'git', 'all', 'none'];
...
const SIMPLE_TOOL_ADAPTERS = [
  ...
  { tool: 'copilot', template: 'copilot-instructions.tmpl', destParts: ['.github', 'copilot-instructions.md'] }
];
...
const VALID_PROFILES = ['minimal', 'standard', 'full', 'strict'];
...
// requested. "standard" (or legacy "full") restores the full guidance corpus
// and test verification. "strict" further enables pre-commit test verification,
// architectural import boundary enforcement, and strict waiver audits.
function resolveProfile(args) {
  const raw = args.preset || args.profile || (args.strict ? 'strict' : 'minimal');
...
function buildAgentsMdSections(profile) {
  if (profile === 'strict') {
    return {
      FIRST_FIVE_MINUTES: [
        ...
        '3.  **Check guardrails:** Review `.agent-room/guardrails.md`. Strict mode is',
        '    enforced: pre-commit test verification, import boundaries, and strict waivers.',
      ...
      GUIDANCE_LINKS: [
        ...
        '  constraints. Pre-commit tests and import boundaries are mechanically enforced.',
        ...
        '  `session-log-format`.',
        '',
        '_Scaffolded with `--preset strict`: pre-commit test execution, import boundaries,',
        'and strict waiver audits are actively enforced._'
      ].join('\n'),
      DEFAULT_WORKFLOW: [
        ...
        '   not a suggestion. Follow `.agent-room/skills/closing-the-loop.md`. Waivers',
        '   in strict mode require an explicit audit reference (approved-by:, ticket: #).',
        '   If nothing qualifies, record the decision or anti-pattern.'
      ].join('\n')
    };
  }

  if (profile === 'full' || profile === 'standard') {
    return {
      FIRST_FIVE_MINUTES: [
        '1.  **Check coordination state:** Are there other agents working? Check open',
        '    PRs, issue assignments, or the `.agent-room/sessions/` directory.',
  ...
  return {
    ...
    DEFAULT_WORKFLOW: [
      '1. **Bug fixes**: Reproduce -> diagnose root cause -> write failing test first',
      '   (TDD) -> minimal fix -> verify. Skips RPI documentation overhead.',
      '2. **Features & non-trivial changes (RPI)**: Follow the Research → Plan → Implement pipeline:',
      '   - **Research**: Run `research-codebase` to map architecture and boundaries (read-only);',
      '     save findings to `docs/research/YYYY-MM-DD-<topic>.md`.',
      '   - **Plan & Iterate**: Run `writing-plans` (or `iterate-plan`) to author a phased checklist',
      '     in `docs/plans/YYYY-MM-DD-<topic>.md`. Get human approval before building.',
      '   - **Implement**: Run `implement-plan` to execute phase-by-phase using TDD, verifying with',
      '     automated commands and checking off items (`- [ ]` -> `- [x]`) on disk.',
      '   - **Audit**: Run `validate-plan` before completion to verify implementation against the plan.',
      '   - **Deliver**: Run `commit-changes` for atomic commits and `describe-pr` for PR summaries.',
      '3. **Verify before claiming done**: run the actual test/build/lint command in',
      '   this turn and read its output before saying "tests pass" or "fixed."',
      '   "Should work" is not evidence.',
      '4. **Serialize state**: Before ending your session, log your work in',
      '   `.agent-room/sessions/`.',
      '5. **Close the loop — before ending the turn, not after**: this is a gate,',
      '   not a suggestion. Follow `.agent-room/skills/closing-the-loop.md`. If the',
      '   turn fixed a bug, found a root cause, or made a non-obvious design call,',
      '   append it to `.agent-room/anti-patterns.md` or `.agent-room/decisions.md`',
      '   *before* claiming the task is done. If nothing qualifies, say so',
...
async function resolveTools(args, detected) {
  if (args.tools) {
    ...
    if (list.includes('all')) {
      return ['claude', 'cursor', 'windsurf', 'cline', 'codex', 'copilot', 'git'];
    }
    return list;
  }
  ...
  const answer = await ask(
    `Which agent tools should get adapters? (comma-separated: claude,cursor,windsurf,cline,codex,copilot,git,all; default: ${defaultStr}): `
  );
...
const CLAUDE_COMMAND_FILES = ['research.md', 'plan.md', 'implement.md', 'iterate.md'];

function installClaudeCommands(target, srcDirs, vars, opts) {
  const results = [];
  for (const cmdFile of CLAUDE_COMMAND_FILES) {
    const relDest = path.join('.claude', 'commands', cmdFile);
    ...
    results.push(
      Object.assign(
        ...
        copyFileInherited(
          srcDirs,
          path.join('adapters', 'claude-commands', cmdFile),
          dest,
          vars,
          opts
        )
...
function computeEnforcedFeatures(results, config) {
  ...
  if (config && (config.preset === 'strict' || config.profile === 'strict')) {
    items.push({
      label: 'Pre-commit test verification gate',
      file: '.agent-room/hooks/guardrails-check.js',
      detail: 'runs verification test command before every commit, blocking commits on test failure'
    });
    items.push({
      label: 'Import boundary guardrails',
      file: '.agent-room/hooks/guardrails-check.js',
      detail: 'blocks commits where source files import test fixtures or violate architectural module boundaries'
    });
    items.push({
      label: 'Strict waiver audit gate',
      file: '.agent-room/hooks/close-the-loop-check.js',
      detail: 'requires audit reference (ticket: #123, approved-by:) and 40+ char justification for waivers'
    });
  }
...
async function runInit(target, args) {
  ...
  try {
    ...
    if (profile === 'strict' && !dryRun) {
      const guardrailsPath = path.join(target, '.agent-room', 'guardrails.json');
      if (fs.existsSync(guardrailsPath)) {
        try {
          const gData = JSON.parse(fs.readFileSync(guardrailsPath, 'utf8'));
          gData.verifyOnCommit = {
            enabled: true,
            strict: true
          };
          gData.strictWaivers = true;
          gData.scopeGuidance = {
            maxFilesPerChange: 10,
            maxLinesPerChange: 300
          };
          gData.importBoundaries = [
            {
              source: 'lib/**',
              disallowed: ['test/**', 'tests/**', 'spec/**', 'fixtures/**'],
              description: 'Source files must not import test files or fixtures'
            },
            {
              source: 'src/**',
              disallowed: ['test/**', 'tests/**', 'spec/**', 'fixtures/**'],
          ...
          fs.writeFileSync(guardrailsPath, JSON.stringify(gData, null, 2) + '\n');
        } catch (err) {
          // ignore
        }
      }
    ...
    if (!fs.existsSync(configPath) || opts.force) {
      const canonicalPreset = profile === 'full' ? 'standard' : profile;
      const configData = {
        name,
        tools,
        language,
      ...
      const effectiveTestCmd =
        resolvedTestCommand ||
        ((canonicalPreset === 'standard' || canonicalPreset === 'strict') &&
        !args['no-test-command'] &&
        !args['skip-test-verification']
          ? testCommand
          : null);
      if (effectiveTestCmd) {
        configData.verification = {
          testCommand: effectiveTestCmd
        };
      }
    ...
    if (tools.includes('claude')) {
      ...
      results.push(...installClaudeCommands(target, srcDirs, vars, opts));
    }
  ...
  console.log(`${bold('Preset:')}           ${cyan(profile === 'full' ? 'standard' : profile)}`);
