...
- Custom eval case formats: supports both standalone `*.eval.json` files and subdirectories with `eval.json`.
- Extended case types: added `verify` case type (running `verifyProject`) and `command` case type (running arbitrary shell scripts or security checks) to evaluate repo-specific policies alongside existing built-in checks.
- Granular eval execution modes: added `--custom-only` to run only custom suites and `--builtin-only` to run only built-ins (by default runs combined suite).
- Multi-format compliance reports: multi-format reporting (`text`, `json`, `csv`) now includes detailed breakdowns of built-in vs custom pass/fail counts (`summary.builtin` and `summary.custom`) and source tags.

- PR Attestation & Verification Evidence Generator (`create-agent-room pr-desc`, Story 3.2): added `--verify` (alias `--with-verification`) to automatically run the project verification test suite and embed durable verification proof, execution duration, and console output into PR descriptions.
- Reviewer Compliance Checklist: automated interactive markdown checklist generated by `pr-desc --verify`, tracking test suite pass status, architectural decisions documentation, and guardrail compliance.
- Guardrails & Decisions PR Attestation: checks `.agent-room/guardrails-bypass-log.md` and `.agent-room/decisions.md` to attest zero bypasses (or document authorized bypasses) and link recent ADRs.
- Custom PR output destination: added `--output <file>` option to `pr-desc` to write generated PR descriptions directly to custom paths (e.g. `.github/pull_request_template.md` or CI artifacts).
- Added `--strict` support to `pr-desc --verify` to exit `1` if test verification fails.
